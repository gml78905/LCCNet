...
from .coarse_context import CoarseContextBuilder, PairRelationBuilder, SharedCoarseContextV2
...
class TriModalJointCalibNetV1(nn.Module):
    """
    v1 architecture main path:
      coarse context -> pair relation -> memory -> reliability -> refined relation -> delta pose

    Notes:
      - Returns delta pose only.
      - Corrected pose must be computed outside this model (loss/util side).
      - Device-safe: no hard-coded .cuda().
      - TODO(v3): integrate equirectangular lidar/radar path under the same forward contract.
    """
    ...
    def __init__(
        ...
        self.coarse_context = CoarseContextBuilder(s32_channels_each=512, d_shared=256, d_mod=256)
        self.pair_relation = PairRelationBuilder(
            s16_channels_each=256,
            s32_prior_dim=512,
            d_shared=256,
            d_pair=384,
            s16_token_dim=256,
        ...
        self.reliability = ReliabilityEstimator(d_mod=256, d_pair=384, d_shared=256, d_pair_mem=256)
    ...
    def _forward_step(
        ...
        coarse = self.coarse_context(cam["s32"], lid["s32"], rad["s32"])
        r_cl_coarse, r_cr_coarse, r_lr_coarse = self.pair_relation(
            cam["s16"],
            lid["s16"],
            rad["s16"],
            coarse["p_cam"],
        ...
        aux = {
            "z_shared_coarse": coarse["z_shared_coarse"],
            "z_cam": coarse["z_cam"],
            "z_lid": coarse["z_lid"],
            "z_rad": coarse["z_rad"],
            "p_cam": coarse["p_cam"],
            "p_lid": coarse["p_lid"],
            "p_rad": coarse["p_rad"],
            "r_cl_coarse": r_cl_coarse,
            "r_cr_coarse": r_cr_coarse,
            "r_lr_coarse": r_lr_coarse,
            "h_shared": h_shared,
            "h_cl": h_cl,
            "h_cr": h_cr,
            "h_lr": h_lr,
